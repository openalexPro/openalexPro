...
- **openalex-snapshot** binary: Required for `snapshot_to_parquet()`, `build_corpus_index()`,
  and `lookup_by_id()`. Download from
  [GitHub releases](https://github.com/rkrug/openalex-snapshot/releases) or build
  with `cargo build --release`. Alternatively, use the pure-R `*_R()` variants.
...
1. **Deletes** the existing parquet directory first (depends on `clean_parquet`)
2. Converts the JSON snapshot to parquet, using the specified worker count and memory limit
3. Supports `PROFILE` values: `safe`, `balanced` (default), `fast`

**NB:** Set `MAX_MEMORY_MB` (memory cap in MB), `WORKERS`, and `PROFILE` to values that work with your system. Since `parquet` unconditionally deletes the existing parquet directory, call `snapshot_to_parquet()` (or `snapshot_to_parquet_R()`) directly in R if you want resume behaviour.
...
This creates a single index parquet file for each entity type, enabling fast lookups by OpenAlex ID:
...
The index maps each ID to its physical location (file and row number) in the corpus, so `lookup_by_id()` can retrieve specific records without scanning the entire corpus.

**NB:** Indexing is less memory-intensive than parquet conversion and profits from higher `WORKERS`. To rebuild indexes without touching the parquet files, use `make clean_index` followed by `make parquet_index`.
...
OpenAlex releases new snapshots regularly. The Makefile supports update workflows via dedicated timestamp targets.
...
This shows the current values of all configuration variables, including `RELEASE_DATE` read from `RELEASE_NOTES.txt`.
...
**Note:** `snapshot_timestamp` and `parquet_timestamp` read the release date from
`RELEASE_NOTES.txt` in the snapshot directory. Run `snapshot_timestamp` *before*
downloading the new snapshot, and `parquet_timestamp` *before* re-converting, so the
old release date is still available.
...
**Out of memory**: Reduce memory usage by overriding variables on the command line:

```bash
make parquet MAX_MEMORY_MB=4000 WORKERS=1 PROFILE=safe
```

**DuckDB errors** (when using `*_R()` fallback functions): Ensure you have the latest version of the `duckdb` R package.

<!-- ## Index Issues

**Slow lookups**: Rebuild the index with fewer workers to reduce the number of files per partition. -->
...
Variables can be overridden on the command line or by editing the defaults at the top of the Makefile:
