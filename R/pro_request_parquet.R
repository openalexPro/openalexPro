...
#'   [future.apply::future_lapply()].  `NULL` or `1` runs sequentially.
#'   Default `NULL`.
...
pro_request_parquet <- function(
  ...
  progressr::with_progress({
    ...
    future.apply::future_lapply(seq_along(jsons), function(i) {
      ...
      #
      # Paginated (results / group_by):
      #   Innermost: unnest the array → one struct column `r`
      #   Middle:    SELECT r.* → expand struct to bare column names
      #   Outer:     SELECT *, <computed cols> → enrich + copy to parquet
      #
      # Single-record:
      #   read_json_auto gives the object as a flat row; enrich directly.
